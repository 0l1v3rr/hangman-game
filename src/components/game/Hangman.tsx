  ...
  return (
    <aside className="bg-slate-400 w-full md:w-1/3 rounded-sm shadow-sm px-4 pt-2 pb-4 bg-opacity-85">
      <div className="flex items-center justify-between gap-2">
        <div className="font-semibold text-zinc-900">You</div>
        <div className="text-zinc-800">{phase}/7</div>
      </div>
      ...
      >
        ...
        {/* hangman itself */}
        {phase > 0 && <div className="absolute bg-zinc-800 w-2 h-10 top-6" />}

        {phase > 1 && (
          <div className="absolute w-12 h-12 top-14 border-8 border-solid border-zinc-800 rounded-full" />
        )}

        {phase > 2 && <div className="absolute bg-zinc-800 w-2 h-24 top-24" />}
